...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$tree_sitter_cli_VERSION-${BUILD_VERSION}~${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "tree-sitter-cli-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "tree-sitter-cli-ubuntu-$dist-$build_arch")"
